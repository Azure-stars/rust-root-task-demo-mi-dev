#let project(
  ...
  set page(numbering: "1", number-align: center, header: align(left)[
    #set text(font: header-font, size: 10.5pt, weight: "regular")
    #title
  ])
  ...
  locate(it => {
    align(center, text(size: 1.5em, font: title-font, weight: "bold")[目录])

    let elements = query(heading.where(outlined: true).after(it), it)

    for el in elements {
      if el.level > 3 or el.numbering == none {
        continue;
      }
      
      v(1em, weak: true)
      
      let title = numbering(el.numbering, ..counter(heading).at(el.location()))+ [
        #h(0.2em)
        #el.body
      ]

      let before_toc = query(heading.where(outlined: true).before(it), it).find((one) => {one.body == el.body}) != none
      ...
      link(el.location())[#{
        ...
        // set underline(false)
        if el.level == 1 {
          set text(font: raw-font, weight: "semibold")
          title
        } else {
          title
        }
