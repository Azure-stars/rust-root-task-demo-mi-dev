...
use sel4::{
    debug_println, reply, with_ipc_buffer_mut, BootInfo, CapRights, Endpoint, LocalCPtr,
...
fn read_item<T: Sized + Copy>(ptr: *const T, cap: LocalCPtr<sel4::cap_type::SmallPage>) -> T {
...
pub(crate) fn handle_ipc(recv_ep: &Endpoint) {
    ...
    let new_cap = LocalCPtr::<sel4::cap_type::SmallPage>::from_bits(DEFAULT_CUSTOM_SLOT + 3);
    with_ipc_buffer_mut(|buf| {
        buf.set_recv_slot(&BootInfo::init_thread_cnode().relative(new_cap));
    });
