...
impl TcpSocket {
    ...
    pub fn recv_timeout(&self, buf: &mut [u8], ticks: u64) -> AxResult<usize> {
        ...
        //             // It need to get timer ticks to check timeout.
        //             unimplemented!()
        //         }
        //     })
        // })
...
impl TcpSocket {
    ...
    where
        F: FnMut() -> AxResult<T>,
    {
        debug!("TCP socket {}: blocking on", self.handle.get() as usize);
        if self.is_nonblocking() {
            f()
        } else {
            loop {
                ...
                match f() {
                    Ok(t) => return Ok(t),
                    Err(AxError::WouldBlock) => sel4::r#yield(),
                    Err(e) => return Err(e),
                }
            }
        }
...
fn get_ephemeral_port() -> AxResult<u16> {
    ...
    let mut tries = 0;
    // TODO: more robust
    debug_println!("Can listen: {}", LISTEN_TABLE.can_listen(6379));
