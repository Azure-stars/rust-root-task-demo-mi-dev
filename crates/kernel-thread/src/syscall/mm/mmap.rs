...
pub(crate) fn sys_mmap(
    ...
    let permission_flags = MmapProt::from_bits(prot).ok_or(Errno::EINVAL)?;
        ...
        .expect("Failed to create page iterator")
    {
        ...
        let page_cap = crate::object_allocator::alloc_cap::<sel4::cap_type::Granule>();
        debug_println!("vaddr: {:?}, page_cap: {:?}", vaddr, page_cap);
        task.map_page(vaddr.as_usize(), page_cap, permission_flags.clone().into());
    }
