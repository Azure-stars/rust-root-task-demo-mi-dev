...
use sel4::{debug_println, with_ipc_buffer_mut, BootInfo, CapRights, LocalCPtr, VMAttributes};

use crate::{object_allocator::alloc_cap, page_seat_vaddr};

fn send_net_ipc(label: NetRequsetabel, cap: Option<LocalCPtr<sel4::cap_type::SmallPage>>) {
    let ipc_ep = LocalCPtr::<sel4::cap_type::Endpoint>::from_bits(DEFAULT_CUSTOM_SLOT + 2);
...
fn gen_cap<T: Sized + Copy>(
    item: *const T,
    num: Option<usize>,
) -> LocalCPtr<sel4::cap_type::SmallPage> {
    let new_cap = alloc_cap::<sel4::cap_type::Granule>();
    new_cap
        .frame_map(
