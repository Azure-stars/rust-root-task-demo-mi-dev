...
// fn read_item<T: Sized + Copy>(task: &mut Sel4Task, addr: *const T) -> T {
//     let buf_addr = VirtAddr::from_ptr_of(addr);
//     return if let Some(cap) = task.mapped_page.get(&align_bits(buf_addr.as_usize(), 12)) {
...
//             )
//             .unwrap();
//         let vaddr = VirtAddr::from_ptr_of(addr);
//         if vaddr.align_offset_4k() + core::mem::size_of::<T>() > PAGE_SIZE_4K {
//             panic!("Item size is too large");
//         }
//         let item = unsafe {
//             core::ptr::read_volatile(
//                 (crate::page_seat_vaddr() + vaddr.align_offset_4k()) as *const T,
//             )
//         };
...
// fn write_item<T: Sized + Copy>(task: &mut Sel4Task, addr: *mut T, item: T) {
//     let buf_addr = VirtAddr::from_ptr_of(addr);
...
//                 (crate::page_seat_vaddr() + vaddr.align_offset_4k()) as *mut T,
//                 item,
//             )
//         };
//         new_cap.frame_unmap().unwrap();
